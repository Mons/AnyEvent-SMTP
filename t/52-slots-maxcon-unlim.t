...
$AnyEvent::SMTP::Client::MAXCON = 1;
$AnyEvent::SMTP::Client::MAXCON{'127.0.0.1'} = 0;
...
for (1..10) {
	sendmail
		...
		cv     => $cv,
		cb     => sub {
			$max = $AnyEvent::SMTP::Client::ACTIVE if $AnyEvent::SMTP::Client::ACTIVE > $max;
...
cmp_ok($max, '>', $AnyEvent::SMTP::Client::MAXCON,
	'max active '.$max.' > '.$AnyEvent::SMTP::Client::MAXCON);
